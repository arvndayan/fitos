...
async function resolveParsedFood(parsed){
  ...
  const local=rankLocalFoods(query,FOOD_SEED,state.usage);
  if(local.length){renderMatches(local.slice(0,5),parsed);return}

  $('#nutritionStatus').textContent='Searching the FitOS food database…';
  try{
    const backend=await searchFitOSBackend(query,20);
    if(backend.length){renderMatches(backend,parsed);return}
  }catch(err){
    console.warn('FitOS backend search unavailable:',err);
  }

  $('#nutritionStatus').textContent='Searching packaged-food fallback…';
  try{
    const external=await searchOpenFoodFacts(query,8);
    ...
    $('#nutritionStatus').textContent='Food search is unavailable right now. Try a local seed food or configure the FitOS backend.';
  }
}
