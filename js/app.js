...
import {rankLocalFoods,searchFitOSBackend,searchOpenFoodFacts} from './nutrition/food-search.js';
...
function normalizeQuery(parsed,choice){
  ...
  if(choice){
    q=`${q} ${choice.toLowerCase()}`;
  }

  if(parsed.preparation){
    q=`${q} ${parsed.preparation}`;
  }
...
function showAmbiguity(a){
  ...
  box.innerHTML=`
    ...
    <div>
      ${a.options
        .map(o=>`<button class="chip" data-choice="${o}">${o}</button>`)
        .join('')}
    </div>
  `;
  ...
  box.querySelectorAll('[data-choice]').forEach(btn=>{
    btn.onclick=()=>{
      ...
      if(a.type==='chicken_cut'){
        pendingParsed.query=
          `${pendingParsed.query} ${choice.toLowerCase()}`;
...
1. FitOS backend
   ...
   -> Open Food Facts backend provider

2. Local development seed

3. Browser Open Food Facts fallback

This prevents old local seed items such as
"Large Whole Egg" from overriding backend results.
*/
...
async function resolveParsedFood(parsed){
  ...
  /*
  ----------------------------------------
  1. SEARCH FITOS BACKEND FIRST
  ----------------------------------------
  */

  $('#nutritionStatus').textContent=
    'Searching the FitOS food database…';

  try{
    const backend=await searchFitOSBackend(query,20);

    if(Array.isArray(backend) && backend.length){
      renderMatches(backend,parsed);
      return;
    }

  }catch(err){
    console.warn(
      'FitOS backend search unavailable:',
      err
    );
  }
  ...
  /*
  ----------------------------------------
  2. LOCAL DEVELOPMENT SEED FALLBACK
  ----------------------------------------
  */
  ...
  /*
  ----------------------------------------
  3. OPEN FOOD FACTS BROWSER FALLBACK
  ----------------------------------------
  */

  $('#nutritionStatus').textContent=
    'Searching packaged-food fallback…';
  ...
  try{
    ...
    $('#nutritionStatus').textContent=
      'No reliable match yet. Try a more specific food name.';

  }catch(err){
    console.warn(
      'Open Food Facts search unavailable:',
      err
    );
...
function renderMatches(foods,parsed){
  ...
  $('#nutritionStatus').textContent=
    'Choose the closest match. FitOS calculates nutrients automatically.';
    ...
    .forEach(btn=>{
      ...
      btn.onclick=()=>{
        ...
        try{
          ...
          $('#nutritionStatus').textContent=
            `Logged ${entry.quantity} ${entry.unit} ${entry.name} (${entry.grams} g).`;
...
function renderFood(){
  ...
  $('#nutCalories').textContent=
    Math.round(
      t.calories||0
    );


  $('#nutProtein').textContent=
    (t.protein||0)
      .toFixed(1);


  $('#nutCarbs').textContent=
    (t.carbs||0)
      .toFixed(1);
        ...
        <div class="row">
          ...
          <div>
            ...
            <div class="muted">
              ...
              ${e.meal}
              ·
              ${Math.round(e.nutrition.calories||0)}
              kcal
              ·
              ${(e.nutrition.protein||0).toFixed(1)}
              g protein
            </div>
...
function renderDashboard(){
  ...
  $('#todayProtein').textContent=
    Math.round(
      t.protein||0
    );


  $('#todayCalTarget').textContent=
    state.targets.calories;


  $('#todayProteinTarget').textContent=
    state.targets.protein;


  $('#todayCalBar').style.width=
    `${
      Math.min(
        100,
        (t.calories||0)
  ...
  $('#todayWeight').textContent=
    state.weights.at(-1)?.value
    ||
...
$('#coachSend').onclick=()=>{
  ...
  let a=
    'I can use your logged FitOS data to help.';
  ...
  if(
    ...
    a=
      `You have about ${
        ...
        .toFixed(0)
      } g protein remaining. Build the next meal around a lean protein source, vegetables and a carb portion that fits the remaining calories.`;
    ...
    a=
      `Your selected program is ${state.program}. Open Training to see today's sessions.`;
...
function addBubble(text,type){
  ...
  $('#coachMessages').scrollTop=
    $('#coachMessages').scrollHeight;
