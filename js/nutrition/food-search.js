...
export async function searchOpenFoodFacts(query,pageSize=12){
  ...
  return (data.products||[]).map(p=>({
    ...
    per100g:{
      ...
      protein:n(p.nutriments?.proteins_100g),carbs:n(p.nutriments?.carbohydrates_100g),
      fat:n(p.nutriments?.fat_100g),fiber:n(p.nutriments?.fiber_100g),
      sugar:n(p.nutriments?.sugars_100g),saturatedFat:n(p.nutriments?.['saturated-fat_100g']),
      sodium:mg(p.nutriments?.sodium_100g),potassium:mg(p.nutriments?.potassium_100g),
      cholesterol:mg(p.nutriments?.cholesterol_100g),calcium:null,iron:null
    }
  }));
}
